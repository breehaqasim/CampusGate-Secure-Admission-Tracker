...
| F-24 | ZAP 10021 — `X-Content-Type-Options` missing | Added `X-Content-Type-Options: nosniff` in `nginx-csp.inc` (included on all `location` blocks) | *commit hash updated after `nosniff` change — see repo history* | — |
...
| F-20 | ZAP 10055 — CSP wildcard (`img-src` / `media-src`) | Accepted risk — Medium CSP warnings may remain | [DAST — OWASP ZAP](https://github.com/breehaqasim/CampusGate-Secure-Admission-Tracker/actions/workflows/dast.yml) — artifact **`dast-zap-reports`** (`report.json`); human review vs §6.1 |
| F-21 | ZAP 10055 — `style-src` / inline styles | Accepted risk — Medium CSP warnings may remain | [DAST — OWASP ZAP](https://github.com/breehaqasim/CampusGate-Secure-Admission-Tracker/actions/workflows/dast.yml) — artifact **`dast-zap-reports`** (`report.json`); human review vs §6.1 |
...
| F-24 | ZAP 10021 — `X-Content-Type-Options` missing | Open — Low alert may still appear | [DAST — OWASP ZAP](https://github.com/breehaqasim/CampusGate-Secure-Admission-Tracker/actions/workflows/dast.yml) — artifact **`dast-zap-reports`** (`report.json`) |
